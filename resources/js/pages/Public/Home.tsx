...
export default function Home({ villageInfo, latestNews, announcements, stats }) {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            setCurrentStat((prev) => (prev + 1) % subStats.length);
        }, 5000);
        return () => clearInterval(timer);
    }, []);

    const nextStat = () => setCurrentStat((prev) => (prev + 1) % subStats.length);
    const prevStat = () => setCurrentStat((prev) => (prev - 1 + subStats.length) % subStats.length);
        ...
        <PublicLayout villageInfo={villageInfo}>
                            ...
                            <div key={currentStat} className="flex-1 flex flex-col md:flex-row items-center justify-between w-full animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
                                ...
                                <div className="w-full md:w-1/3 text-center md:text-left mb-8 md:mb-0">
                                    <h3 className="text-2xl md:text-3xl font-bold text-[#EFA00B] leading-tight">
                                        {subStats[currentStat].title}
                                    </h3>
                                    {subStats[currentStat].subtitle && (
                                ...
                                <div className="w-full md:w-1/3 flex justify-center mb-8 md:mb-0 relative">
                                    <div className="w-48 h-48 md:w-56 md:h-56 relative animate-in zoom-in-95 duration-700 ease-out fill-mode-both" style={{ animationDelay: '100ms' }}>
                                        {subStats[currentStat].type === 'landmark' ? (
                                            <div className="w-full h-full flex items-center justify-center bg-white/50 rounded-full p-2 backdrop-blur-sm shadow-sm">
                                                <img src={subStats[currentStat].image} className="w-full h-full object-contain drop-shadow-lg transform transition-transform scale-150 hover:scale-[1.6]" alt="Landmark" />
                                            </div>
                                        ) : (
                                ...
                                <div className="w-full md:w-1/3 flex flex-col justify-center gap-6 pl-0 md:pl-8 animate-in slide-in-from-right-8 duration-500 ease-out" style={{ animationDelay: '200ms' }}>
                                    {subStats[currentStat].data.map((item, idx) => (
            ...
            <section className="py-20 bg-[#FFFDF7]">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <h2 className="text-3xl font-bold text-slate-800 mb-16">Struktur Organisasi dan Tata Kerja</h2>

                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-8">
                        {[
                            { name: 'Asep Saefullah', role: 'Staff', gender: 'male' },
                            { name: 'Aulia Namesti', role: 'Staff', gender: 'female' },
                            { name: 'Nabila Aminatun', role: 'Staff', gender: 'female' },
                            { name: 'Bintang Shailahudin', role: 'Staff', gender: 'male' },
                            { name: 'David Aditya', role: 'Staff', gender: 'male' },
                            { name: 'Daffa Hafid', role: 'Staff', gender: 'male' },
                        ].map((person, idx) => (
                            <div key={idx} className="flex flex-col items-center group">
                                <div className="w-24 h-24 mb-4 overflow-hidden rounded-full border-4 border-white shadow-lg group-hover:scale-110 transition-transform duration-300">
                                    <img
