...
export default function HeroEdit({ images }: { images: any[] }) {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            console.log('Hero Image Selected:', {
                name: file.name,
                size: file.size,
                type: file.type,
            ...
            if (file.size > 15728640) {
                ...
                alert(`File terlalu besar (${(file.size / 1024 / 1024).toFixed(2)} MB). Maksimal 15MB.`);
                return;
            }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        console.log('Hero Upload Started:', {
            hasImage: !!data.image,
            title: data.title,
        ...
        post(route('admin.hero.store'), {
            onSuccess: () => {
                console.log('Hero Upload Success');
                reset();
                setPreview(null);
            },
            onError: (errors) => {
                console.error('Hero Upload Error:', errors);
                // Show alert for better visibility
                const errorMessage = errors.image || Object.values(errors)[0] || 'Upload gagal';
                alert('Error Upload: ' + errorMessage);
            },
            onFinish: () => {
                console.log('Hero Upload Finished');
            }
        });
    };
                            ...
                            <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <h3 className="text-sm font-semibold text-red-800">Upload Gagal</h3>
                                        <p className="text-sm text-red-700 mt-1">{errors.image}</p>
                                    </div>
                                </div>
                            </div>
                        )}
